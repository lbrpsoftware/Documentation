...
### 2.3 Organizations/Applications Contracts
...
`TOneOf<TFail, TOkay>` is a generic record that can contain one of two possible values: an **error value** (`TFail`) or a **success value** (`TOkay`). This pattern is used as an alternative to throwing exceptions in API calls — instead of throwing an exception on a failed call, the function returns a `TOneOf` that contains either a `TGeneralFailResponse` (error) or a success type.

The internal field `FIndex` indicates which value is active:
- `0` → not set
- `1` → `TFail` (error)
- `2` → `TOkay` (success)
...
| Method | Description |
|---|---|
| `From(TFail)` | Creates a `TOneOf` with an error value (`FIndex = 1`) |
| `From(TOkay)` | Creates a `TOneOf` with a success value (`FIndex = 2`) |
| `IsFail` | Returns `True` if the value is an error |
| `IsOkay` | Returns `True` if the value is a success |
| `IsSet` | Returns `True` if a value is set (error or success) |
| `AsFail` | Returns the error value; throws an exception if it is not an error |
| `AsOkay` | Returns the success value; throws an exception if it is not a success |
| `TryGetFail` | Safe variant of `AsFail` — returns `False` if it is not an error |
| `TryGetOkay` | Safe variant of `AsOkay` — returns `False` if it is not a success |

### Usage in practice

After an API call you check the result as follows:
...
Or with the safe `TryGet` variants:
